...
with tab_economics:
    ...
    st.markdown('<div class="section-label">Crop-Year Realized Price per Bag (Jul&ndash;Jun)</div>',
                unsafe_allow_html=True)
    st.markdown(
        seasonal_table_html(econ.econ_wide(df_econ, econ.COLS["Arabica"]["price"]), year_cols_econ,
                             title="Arabica Realized Price", unit="$/bag", kind="ratio"),
        unsafe_allow_html=True,
    )
    st.markdown(
        seasonal_table_html(econ.econ_wide(df_econ, econ.COLS["Robusta"]["price"]), year_cols_econ,
                             title="Robusta Realized Price", unit="$/bag", kind="ratio"),
        unsafe_allow_html=True,
    )

    st.markdown('<div class="section-label">Physical Differential vs Exchange</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="card-desc">Luis\'s own weighted physical/FOB differential vs the NY (Arabica) or '
        'London (Robusta) contract &mdash; his figure, not derived from the Cecafe revenue/volume data '
        'above. Only populated from 2014&ndash;present, well short of the revenue/price tables\' '
        '1990-present history.</div>',
        unsafe_allow_html=True,
    )
